...
In plain English, the score is computed by:
multiplying the number of characters divided by the number of words by 4.17,
...
def ari_scan(file_name):
    text = open(file_name, "r")
    readable = text.read()
    text.close()
    # print(readable)
    sents = re.split(r'[!?.]', readable)
    # print(sents)
    num_sent = len(sents)
    print("sentences:", num_sent)
    words = re.split(r'[\s\n!?.,;:"“”(--)/]', readable)
    ...
    # print(num_words)
    # print(num_sent)
    ari_score = ((num_characters / num_words) * 4.17) + (.5 * (num_words / num_sent)) - 21.43
    # print(ari_score)
    ari_scale = {
         1: {'ages':   '5-6', 'grade_level': 'Kindergarten'},
         2: {'ages':   '6-7', 'grade_level':    '1st Grade'},
    ...
    ari_score_rounded = math.ceil(ari_score)
    return ("\nThe ARI for the file {} is {}. \nThis corresponds to a {} level of difficulty \nthat is suitable for \
an average person {} years old.\n\n".format(file_name, ari_score_rounded, ari_scale[ari_score_rounded]['grade_level'],
                                            ari_scale[ari_score_rounded]['ages']))
...
print(ari_scan('grimms.txt'))
print(ari_scan('the_yellow_wallpaper.txt'))
print(ari_scan('pride_and_prejudice.txt'))